...
def serve_json():
    return send_from_directory('/var/www/html', 'tesla.json', mimetype='application/json'), 200
...
# 2. CONFIGURAZIONE NGINX (Priorità Corretta)
RUN echo 'server { \
    listen ${PORT}; \
    \
    # Priorità 1: Rotte critiche (Flask) \
    ...
    \
    # Priorità 2: Tutto il resto (Proxy Go) \
    location / { \
